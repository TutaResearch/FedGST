...
def unscaled_metrics(y_pred, y, scaler, name):
    ...
    # y_pred[y_pred[..., 0:140]<=0] = 1
    # y_pred[y_pred[..., 0:140]>0] = 0
    # mse
    mse = ((y_pred - y) ** 2).mean()
    # RMSE
    rmse = torch.sqrt(mse)
    ...
    # # print(y_pred)
    y = y.reshape((-1, 140))
    y_pred = y_pred.reshape((-1, 140))
    ndcg = torch.tensor(ndcg_score(y, y_pred))
    ndcg5 = torch.tensor(ndcg_score(y, y_pred, k = 5))
    ndcg10 = torch.tensor(ndcg_score(y, y_pred, k = 10))
    ndcg20 = torch.tensor(ndcg_score(y, y_pred, k = 20))
    ...
    return {
        ...
        '{}/ndcg'.format(name): ndcg.detach(),
        '{}/ndcg5'.format(name): ndcg5.detach(),
        '{}/ndcg10'.format(name): ndcg10.detach(),
        '{}/ndcg20'.format(name): ndcg20.detach()
