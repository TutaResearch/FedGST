...
class SplitFedNodePredictor(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.hparams.mp_worker_num <= 1:
            ...
            #deletion method of bad influence
            goodResults = list()
            b = list()
            for i in range(len(local_train_results)):
              a = list()
              for j in range(len(local_train_results[i])):
                # a[i][j] = local_train_results[i][j]['log']['train/loss']
                a.append(local_train_results[i][j]['log']['train/loss'].item())
              mean_train_loss = sum(a) / len(a)
              b.append(mean_train_loss)
            print('Influence\n')
            # mu = np.max(np.array(b)) - np.min(np.array(b))
            # print(1 - ((np.array(b) - np.min(np.array(b))) / mu))
            print((np.array(b) / np.sum(np.array(b))))
            # print(torch.softmax(torch.tensor(np.array(b)), 0))
            mean_local_loss = sum(b) / len(b)
            for i, j in enumerate(b):
              if(j < 1.1 * mean_local_loss):
                goodResults.append(local_train_results[i])
            
            local_train_results = goodResults
